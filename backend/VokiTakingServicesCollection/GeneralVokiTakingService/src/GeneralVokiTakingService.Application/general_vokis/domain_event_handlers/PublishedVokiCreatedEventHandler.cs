...
namespace GeneralVokiTakingService.Application.general_vokis.domain_event_handlers;
...
internal class PublishedVokiCreatedEventHandler : IDomainEventHandler<PublishedVokiCreatedEvent>
{
    ...
    public async Task Handle(PublishedVokiCreatedEvent e, CancellationToken ct) {
        var deleteRes = await _mainStorageBucket.DeleteUnusedVokiKeys(
            e.VokiId, e.VokiContentKeys
        );
