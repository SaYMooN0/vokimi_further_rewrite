<script lang="ts">
	import CoAuthorsMessageTemplate from './c_co_authors_msgs_shared/CoAuthorsMessageTemplate.svelte';

	interface Props {
		openInviteDialog: () => void;
	}
	let { openInviteDialog }: Props = $props();
</script>

<CoAuthorsMessageTemplate
	{subtitle}
	title="No co-authors yet"
	icon={groupIcon}
	actionItem={{ type: 'button', text: 'Invite first co-author', onClick: openInviteDialog }}
/>

{#snippet subtitle()}
	This Voki doesn’t have any co-authors<br />Invite the first one to start collaborating!
{/snippet}
{#snippet groupIcon()}
	<svg class="group-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none">
		<path
