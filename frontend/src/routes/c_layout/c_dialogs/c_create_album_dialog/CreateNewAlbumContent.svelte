<script lang="ts">
	...
	let mainColorInput = $state('#fabcbf');
	let secondaryColorInput = $state('#1abcff');
	let useTwoColors = $state(false);
	let secondaryColorWithIfTwoCheck = $derived(useTwoColors ? secondaryColorInput : mainColorInput);
	let mainColorInputLabel = $derived(useTwoColors ? 'Main color' : 'Album color');
	...
	async function saveNewAlbum() {
		...
		if (StringUtils.isNullOrWhiteSpace(mainColorVal)) {
			savingErrs.push({ message: 'Main color is invalid. Choose it using the color picker' });
		}
		if (StringUtils.isNullOrWhiteSpace(secondaryColorVal)) {
			savingErrs.push({ message: 'Secondary color is invalid. Choose it using the color picker' });
		}
	...
	export function reset() {
		name = '';
		icon = 'albums-bookmark-1-icon';
		mainColorInput = '#5B57E2';
		secondaryColorInput = '#bfbfbf';
		useTwoColors = false;
	}
</script>

<div
	class="create-new-album-container"
	onkeydown={(e) => {
		if (e.key === 'Enter') {
			e.preventDefault();
			saveNewAlbum();
...
>
	<div class="header">
		...
		<p class="subtitle">You can always change it later</p>
	</div>
	...
	<button
		class="surprise-btn unselectable"
		onclick={setRandom}
		aria-label="Randomize icon and colors"
	>
		Surprise me
...
<style>
	...
	.surprise-btn:hover svg {
		transform: scale(1.2) rotate(10deg) translateX(3px);
	}

	.surprise-btn:active svg {
	...
	.create-new-album-container > :global(.saving-err-block) {
		min-height: 3rem;
		margin: 0.5rem 0;
	}

	.create-new-album-container > :global(.primary-btn) {
		align-self: center;
		width: 20rem;
		letter-spacing: 1px;
	}
