<script lang="ts">
	...
	async function save() {
		...
		if (response.isSuccess) {
			...
			album.icon = response.data.icon;
			album.mainColor = response.data.mainColor;
			album.secondaryColor = response.data.secondaryColor;
		} else {
...
<style>
	...
	.surprise-btn:hover svg {
		transform: scale(1.2) rotate(10deg) translateX(0.1875rem);
	}
